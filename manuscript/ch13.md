...
```
This page doesn't trigger any effects on init. It also reads the state from the main posts page, without going to the local storage.
...
Hyperapp may get its own dedicated client-side router in future, but the section is still relevant.
...
```
`page(location)` navigates to a given URL. `Navigate` action maintains the current state
and triggers the navigation effect. 
