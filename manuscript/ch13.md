...
Add the `Navigate` effectful action to **Router.js**:
```js
...
```
`Navigate` action maintains the current state
and triggers the navigation effect. The effect is not calling dispatch, but our 3rd party library `page(location)`.
