...
You can visualize your app as the state in the center. Actions and view declarations are sitting around it. And effects/subscriptions at the edges.
...
```
`WithGuid` is a function creating effectful action. `Guid` itself is an effect. The only difference with the `Http` effect is that you don't need any additional `data` so you're passing action directly. 
```js
const Guid = (action) => [
  (dispatch, action) => {
